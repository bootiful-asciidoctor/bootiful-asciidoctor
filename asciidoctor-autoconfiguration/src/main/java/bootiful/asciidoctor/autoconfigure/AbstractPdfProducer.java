...
abstract class AbstractPdfProducer implements DocumentProducer {
	...
	protected File[] optimizeOutputPdf(File file) {
		...
		var process = new ProcessBuilder().command("asciidoctor-pdf-optimize", file.getAbsolutePath()).inheritIO()
				.directory(file.getParentFile()).start();

		var exitCode = process.waitFor();

		/*
		 * var process = Runtime.getRuntime() // .exec("asciidoctor-pdf-optimize " +
		 * file.getAbsolutePath()
		 *//*
			 * todo make this work again + " --quality " + configuration.
			 * getPdfOptimizerQuality().name(). toLowerCase()
			 *//*
				 * ); var exitCode = process.waitFor();
				 */
		if (log.isDebugEnabled()) {
	...
	public File[] produce() {
		...
		if (optimize) {
			File compressedFile = new File(this.properties.getRoot(),
					"index-optimized-" + media.toLowerCase() + ".pdf");
